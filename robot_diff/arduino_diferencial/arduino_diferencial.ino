...
Motor right(9,6,20,21); //(IN1, IN2, verde-20, naraja-21)
Motor left(10,7,2,3);  //(IN3, IN4,  verde-2, naranja-3)  
...
double left_kp = 5, left_ki = 0  , left_kd = 0.025;             
double right_kp = 5, right_ki = 0 , right_kd = 0.025;
...
// Variables de mensaje para los valores de salida del PID
std_msgs::Float32 left_output_msg;
ros::Publisher pleft_wheel_pub("pidlwheel", &left_output_msg);
std_msgs::Float32 right_output_msg;
ros::Publisher pright_wheel_pub("pidrwheel", &right_output_msg);
...
void setup() {
  ...
  nh.advertise(pleft_wheel_pub); 
  nh.advertise(pright_wheel_pub);  
...
void loop() {
  ...
  if (currentMillis - prevMillis >= 10){
    ...
    demand_speed_left = demandx - (demandz*0.2); //es longitud entre llantas 
    ...
    left_setpoint = demand_speed_left*1.13;  
    right_setpoint = demand_speed_right*1.13;
    ...
    leftPID.Compute();
    left.rotatel(left_output);
    rightPID.Compute();
    right.rotater(right_output);
...
void publishOutputs() {
  left_output_msg.data = left_output;
  right_output_msg.data = right_output;
  pleft_wheel_pub.publish(&left_output_msg);
  pright_wheel_pub.publish(&right_output_msg);
}
